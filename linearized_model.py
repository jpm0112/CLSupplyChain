...
# for constraint 4
model.aux_capacity_collection_centres_constraint = pyo.Var(model.collection_centres,model.architectures,model.components, domain=pyo.NonNegativeReals)
...
model.aux_c_ar = pyo.Var(model.reprocessing_centres,model.retailers,model.architectures, domain=pyo.NonNegativeReals)

model.aux_a_ar = pyo.Var(model.reprocessing_centres,model.plants,model.architectures, domain=pyo.NonNegativeReals)
model.aux_b_ar = pyo.Var(model.collection_centres,model.retailers,model.architectures, domain=pyo.NonNegativeReals)
...
for m in model.collection_centres:
        for c in model.components:
            for a in model.architectures:
                    ...
                    model.capacity_collection_centres_constraints.add(model.aux_capacity_collection_centres_constraint[m,a,c] * bill_of_materials[a,c]
                                                                      + sum(model.f[m, r, c] for r in model.reprocessing_centres)
                                                                      <= collection_centres_capacity[m, c] * model.opm[m])

                    model.capacity_collection_centres_constraints.add(model.aux_capacity_collection_centres_constraint[m, a, c] <= big_m * model.ar[a] )
                    model.capacity_collection_centres_constraints.add(model.aux_capacity_collection_centres_constraint[m,a,c] <=
